#!/usr/bin/env python3
"""
ProPublica Nonprofit API Integration
Real-time access to 1.8 million nonprofits
NO API KEY REQUIRED!
"""
...
@dataclass
class NonprofitSummary:
    """Basic nonprofit information from search"""
    ein: str
    name: str
    city: str
    state: str
...
@dataclass
class NonprofitDetail:
    """Detailed nonprofit information"""
    ein: str
    name: str
    city: str
    state: str
    ...
    program_expense_percent: Optional[float]
    fundraising_expense_percent: Optional[float]
...
class ProPublicaAPI:
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            'User-Agent': 'Neil Brockway Partner Finder 1.0'
        })
    
    def search_by_name(self, query: str, state: str = None) -> List[NonprofitSummary]:
        """
        ...
        try:
            ...
            for org in data.get('organizations', []):
                results.append(NonprofitSummary(
                    ein=str(org['ein']),
                    name=org['name'],
                    city=org.get('city', ''),
                    state=org.get('state', ''),
    ...
    def search_by_zip(self, zip_code: str, radius_miles: int = 10) -> List[NonprofitSummary]:
        """
        Search nonprofits near a ZIP code
        Note: ProPublica doesn't have direct ZIP search, so we search by state
        """
        # For now, we'll need to map ZIP to state
        state = self._zip_to_state(zip_code)
        if not state:
            print(f"Cannot determine state for ZIP {zip_code}")
            return []
        
        # Search all nonprofits in that state
        params = {'state': state}
        ...
        try:
            ...
            results = []
            for org in data.get('organizations', [])[:50]:  # Limit to 50 for demo
                results.append(NonprofitSummary(
                    ein=str(org['ein']),
                    name=org['name'],
                    city=org.get('city', ''),
    ...
    def get_details(self, ein: str) -> Optional[NonprofitDetail]:
        """
        Get detailed information about a specific nonprofit
        """
        try:
            # Clean EIN (remove hyphens)
            ein_clean = ein.replace('-', '')
            
            response = self.session.get(f"{self.BASE_URL}/organizations/{ein_clean}.json")
            response.raise_for_status()
            data = response.json()
            
            org = data.get('organization', {})
            
            # Get latest filing if available
            latest_filing = None
            ...
            fundraising_expense_pct = None
            filing_year = None
            
            if latest_filing:
                revenue = latest_filing.get('totrevenue')
                expenses = latest_filing.get('totfuncexpns')
                ...
                if expenses and expenses > 0:
                    ...
                    fundraising_exp = latest_filing.get('fundrasing', 0)
                    
                    if program_exp:
                        program_expense_pct = (program_exp / expenses) * 100
                    if fundraising_exp:
                        fundraising_expense_pct = (fundraising_exp / expenses) * 100
            
            return NonprofitDetail(
                ein=ein,
                name=org.get('name', ''),
                city=org.get('city', ''),
                state=org.get('state', ''),
                ...
                expenses=expenses,
                assets=assets,
                program_expense_percent=program_expense_pct,
                fundraising_expense_percent=fundraising_expense_pct,
                latest_filing_year=filing_year
            )
            
        except Exception as e:
            print(f"Details error for EIN {ein}: {e}")
            return None
    
    def _zip_to_state(self, zip_code: str) -> Optional[str]:
        """
        Simple ZIP to state mapping
        In production, you'd use a proper ZIP database
        """
        zip_prefix = zip_code[:3]
        
        # Basic mapping - covers major areas
        zip_map = {
            # Northeast
            ...
            # Midwest
            '400': 'KY', '401': 'KY', '402': 'KY', '403': 'KY', '404': 'KY', '405': 'KY', '406': 'KY', '407': 'KY', '408': 'KY', '409': 'KY', '410': 'KY', '411': 'KY', '412': 'KY', '413': 'KY', '414': 'KY', '415': 'KY', '416': 'KY', '417': 'KY', '418': 'KY', '421': 'KY', '422': 'KY', '423': 'KY', '424': 'KY', '425': 'KY', '426': 'KY', '427': 'KY',
        ...
        return zip_map.get(zip_prefix)


def demo():
    """
    Demo the ProPublica API
    """
    api = ProPublicaAPI()
    
    print("=" * 60)
    print("ProPublica Nonprofit API Demo")
    print("=" * 60)
    
    # Test 1: Search for Red Cross organizations
    print("\n1. Searching for 'Red Cross' organizations...")
    results = api.search_by_name("Red Cross")
    ...
    # Test 2: Get details for American Red Cross
    print("\n2. Getting details for American Red Cross (EIN: 53-0196605)...")
    details = api.get_details("53-0196605")
    
    if details:
        print(f"\n  Name: {details.name}")
        print(f"  Location: {details.city}, {details.state}")
        print(f"  Website: {details.website}")
        if details.mission:
            print(f"  Mission: {details.mission[:150]}...")
        if details.revenue:
            print(f"  Annual Revenue: ${details.revenue:,.0f}")
        if details.expenses:
            print(f"  Annual Expenses: ${details.expenses:,.0f}")
        if details.assets:
            print(f"  Total Assets: ${details.assets:,.0f}")
        if details.program_expense_percent:
            print(f"  Program Expense %: {details.program_expense_percent:.1f}%")
        if details.latest_filing_year:
            print(f"  Latest Filing: {details.latest_filing_year}")
    
    # Test 3: Search by ZIP code
    print("\n3. Searching nonprofits in New York (ZIP 10001)...")
    zip_results = api.search_by_zip("10001")
    
    print(f"   Found {len(zip_results)} organizations in NY")
    for org in zip_results[:5]:
        print(f"   • {org.name} ({org.city})")
    
    print("\n" + "=" * 60)
    print("All data retrieved from ProPublica Nonprofit Explorer")
